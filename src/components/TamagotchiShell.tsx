...
export function TamagotchiShell({ children, onLogout, showButtons = false, buttons }: TamagotchiShellProps) {
  return (
    <div className="min-h-screen h-[100dvh] bg-[#9bbc0f] sm:bg-teal-600 sm:flex sm:items-center sm:justify-center sm:p-4 font-sans selection:bg-[#0f380f] selection:text-[#9bbc0f] overflow-hidden">
      {/* ---------------- TAMAGOTCHI DEVICE SHELL / FULLSCREEN MOBILE ---------------- */}
      <div className="w-full sm:max-w-md bg-[#9bbc0f] sm:bg-rose-500 sm:rounded-[3rem] sm:p-5 sm:shadow-[inset_-4px_-8px_0px_rgba(0,0,0,0.2),0_10px_30px_rgba(0,0,0,0.5)] sm:border-4 sm:border-slate-900 relative flex flex-col h-full sm:h-[95vh] max-h-[100dvh] sm:max-h-none overflow-hidden">
        ...
        <div className="flex justify-between items-center px-4 py-3 pt-safe sm:px-2 sm:py-0 sm:mb-4 bg-[#8bac0f] sm:bg-transparent border-b-4 border-[#0f380f] sm:border-b-0 shrink-0 z-20">
        ...
        {/* ---------------- NAVIGATION BUTTONS (Docked at bottom, safe for iPhone home bar) ---------------- */}
        {showButtons && buttons && (
          <div className="shrink-0 z-20 bg-[#8bac0f] sm:bg-transparent border-t-4 border-[#0f380f] sm:border-t-0 py-2 sm:py-0 pb-safe sm:pb-0 px-3 sm:mt-5 sm:mb-2 flex justify-around items-center">
            {buttons}
          </div>
        )}
      </div>
